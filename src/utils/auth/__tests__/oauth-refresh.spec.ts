...
describe('refreshOAuthToken', () => {
  ...
  it('does not affect discoverOidcMetadata callers that omit a signal (e.g. federated logout)', async () => {
    // discoverOidcMetadata's signal parameter is optional so the federated
    // logout route (which calls it directly, without a refresh-lock timeout
    // budget) keeps working unchanged.
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      jsonResponse(200, { end_session_endpoint: 'https://issuer/logout' }),
    );

    const metadata = await discoverOidcMetadata('https://issuer');

    expect(metadata.end_session_endpoint).toBe('https://issuer/logout');
