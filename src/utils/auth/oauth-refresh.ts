...
export const getAzureB2cIssuer = () => {
...
export const discoverOidcMetadata = async (
  issuer: string,
  signal?: AbortSignal,
) => {
  ...
  if (!response.ok) {
    throw new Error(`Failed to discover OIDC metadata for issuer ${issuer}`);
  }
...
const discoverTokenEndpoint = async (issuer: string, signal: AbortSignal) => {
  const metadata = await discoverOidcMetadata(issuer, signal);
